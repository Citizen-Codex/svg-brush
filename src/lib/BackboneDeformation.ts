...
export class BackboneDeformation {
	 ...
	 */
	static projectPointToBackbone(brushPoint: Point, brush: BrushDefinition): ProjectionResult {
	...
	static deformBrush(brush: BrushDefinition, userPathPoints: Point[]): Point[][] {
		...
		for (const shape of brush.outline.shapes) {
			...
			for (const brushPoint of shape.points) {
				...
				const projection = this.projectPointToBackbone(brushPoint, brush);
	 ...
	 */
	static createDeformedStroke(brush: BrushDefinition, userPathPoints: Point[]): string {
		const deformedShapes = this.deformBrush(brush, userPathPoints);

		if (deformedShapes.length === 0) return '';

		let pathString = '';

		for (let i = 0; i < deformedShapes.length; i++) {
			const shapePoints = deformedShapes[i];
			const isClosed = brush.outline.shapes[i].closed;

			if (shapePoints.length === 0) continue;

			pathString += `M ${shapePoints[0].x} ${shapePoints[0].y}`;

			for (let j = 1; j < shapePoints.length; j++) {
				pathString += ` L ${shapePoints[j].x} ${shapePoints[j].y}`;
			}
